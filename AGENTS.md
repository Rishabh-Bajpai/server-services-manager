...
- **Backend modules:**
  ...
  - `app/file_explorer.py` — chroot-safe file-explorer helpers (Phase 28, extended). `tree()` walks directories with hidden-file + depth + entry caps; `search()` does case-insensitive filename search; `preview()` returns text (utf-8 / latin-1) or base64 image/pdf with a 50 MB cap; `bulk_delete()` and `chmod()` are the batch operations; `make_zip()` bundles a selection into a single zip with a 256 MB total cap; `mkdir()` / `rename()` / `move()` (EXDEV falls back to streamed copy+unlink) / `copy()` (streamed, constant memory) are the cut-copy-paste primitives behind `POST /api/files/{mkdir,rename,move,copy}`. Resumable uploads: `init_upload()` (sparse prealloc, free-space gate, `subpath` mkdir -p for folder uploads) → `append_chunk()` (explicit offset, 1 MiB streaming blocks, fcntl lock, 409-style `offset_mismatch` carrying `received`) → `complete_upload()` (size + optional sha256, atomic rename) / `cancel_upload()`; sessions live as JSON sidecars in `~/.server-services-manager/uploads/`, swept after 48h; routes `POST|GET|PUT|DELETE /api/files/uploads[/<id>[/complete]]`. All entry points use `os.path.normpath` (NOT `realpath`) so symlinks are NOT followed — the explorer's chroot is verified separately for symlinks so a planted link to /etc/passwd is rejected.
