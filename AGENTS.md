...
- **Backend modules:**
  ...
  - `app/file_explorer.py` — chroot-safe file-explorer helpers (Phase 28). `tree()` walks directories with hidden-file + depth + entry caps; `search()` does case-insensitive filename search; `preview()` returns text (utf-8 / latin-1) or base64 image/pdf with a 50 MB cap; `bulk_delete()` and `chmod()` are the batch operations; `make_zip()` bundles a selection into a single zip with a 256 MB total cap. All entry points use `os.path.normpath` (NOT `realpath`) so symlinks are NOT followed — the explorer's chroot is verified separately for symlinks so a planted link to /etc/passwd is rejected.
