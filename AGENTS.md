...
- **Backend modules:**
  ...
  - `app/package_manager.py` — apt/dnf/yum wrapper with security-update detection; manual refresh (`apt update` / `dnf check-update`), cached result with 1h stale threshold; install runs in background with log-tail polling
  - `app/firewall_manager.py` — ufw + firewalld wrapper (auto-detected); status/rules/enable/disable/add/delete/set-default with sudo password auth pattern matching the systemd unit editor
  - `app/backup_manager.py` — Local-disk backup scheduler (directory/mysql/postgres) backed by systemd timer units (`ssm-backup-<name>.{service,timer}`); metadata in `~/.server-services-manager/backups.json`; retention prune via `head -n -N | xargs rm`
  - `app/openapi.py` — OpenAPI 3.0 spec generator (walks `app.url_map`, layers on `@describe()` metadata); `/openapi.json` exposes the spec; `/docs/` serves Swagger UI
  - `app/disk_manager.py` — chroot-safe `du` wrapper for the `/disk` page; runs `du --all --max-depth=N -b` with a 15s timeout; depth capped at 3; absolute paths and traversal rejected via `os.path.realpath()` against `$HOME`. `get_usage()` returns sorted children, `largest_items()` adds human-readable sizes, `breadcrumb()` splits a path into clickable segments.
  - `app/ssh_manager.py` — read/write `~/.ssh/authorized_keys` with RFC 4253 key parsing, SHA256 fingerprint computation (matches `ssh-keygen -lf -E sha256`), atomic write (tempfile + `os.replace`, fsync, chmod 0600), pre-removal backup, lockout protection that refuses to delete the last key unless `confirm_last=true` is passed. Duplicate detection by fingerprint.
  - `app/cluster_manager.py` — manual peer registry at `~/.server-services-manager/cluster.json`; reachability probe (GET /health); API proxy that injects `X-SSM-Cluster-Secret` and forwards the session cookie so the user appears logged in on the remote side. mDNS browse is opt-in (no-op without the optional `zeroconf` package and an explicit `cluster.enabled: true` in config). Safe by default: no auto-broadcast, no background daemons.
  - `app/file_explorer.py` — chroot-safe file-explorer helpers (Phase 28, extended). `tree()` walks directories with hidden-file + depth + entry caps; `search()` does case-insensitive filename search; `preview()` returns text (utf-8 / latin-1) or base64 image/pdf with a 50 MB cap; `bulk_delete()` and `chmod()` are the batch operations; `make_zip()` bundles a selection into a single zip with a 256 MB total cap; `mkdir()` / `rename()` / `move()` (EXDEV falls back to streamed copy+unlink) / `copy()` (streamed, constant memory) are the cut-copy-paste primitives behind `POST /api/files/{mkdir,rename,move,copy}`. `save_text()` backs the in-browser editor via `POST /api/files/save` (editable-allowlist gate, 1 MB cap, atomic write preserving mode bits). Resumable uploads: `init_upload()` (fallocate with sparse fallback, free-space gate, `subpath` mkdir -p for folder uploads) → `append_chunk()` (explicit offset, 1 MiB streaming blocks, per-session fcntl lock across load→write→save, ENOSPC mapped, 409-style `offset_mismatch` carrying `received`) → `complete_upload()` (size + optional sha256, atomic rename with EXDEV copy fallback) / `cancel_upload()`; sessions live as JSON sidecars in `~/.server-services-manager/uploads/`, swept after 48h (orphan `.part` included); routes `POST|GET|PUT|DELETE /api/files/uploads[/<id>[/complete]]`. `_resolve()` canonicalizes the nearest existing ancestor via `realpath()` — a leaf-only `islink` check misses intermediate symlinks (`~/link -> /etc` + `"link/x"`), so every op is verified against the resolved path; `subpath` levels are checked before `makedirs`.
- **Frontend:** All inline JS in `templates/index.html` (~1700 lines, 4 script blocks). Monitor at `templates/monitor.html` uses Chart.js. System services at `templates/system-services.html` (~600 lines, side-panel detail view with Overview/Dependencies/Unit File/Logs tabs). Docker at `templates/docker.html` (containers table + side panel with Details/Stats/Logs tabs, live log streaming). Cron at `templates/cron.html`, notifications at `templates/notifications.html`, activity at `templates/activity.html`, control at `templates/control.html`, disk usage at `templates/disk.html` (squarified treemap + children list + top-20 sidebar), SSH keys at `templates/ssh.html` (paste + list + delete with lockout confirmation), cluster at `templates/cluster.html` (peer cards + manual add + probe/mDNS), full file explorer at `templates/files.html` (tree + breadcrumb + preview + chmod + bulk-select + zip). No build step, no framework.
